...
<div class="h-screen flex flex-col justify-center items-center">
  ...
  <form class="w-80" action="{{route('users.store')}}" method="POST" enctype="multipart/form-data">
    ...
    <div class="flex flex-col gap-4 text-sm">
      @csrf
      <input class="p-2 outline-none rounded-sm border-2 focus:border-blue-600 transition-all" type="text" name="username" required placeholder="Nome de usuário">

      <input class="p-2 outline-none rounded-sm border-2 focus:border-blue-600 transition-all" type="email" name="email" required placeholder="E-mail">

      <input class="p-2 outline-none rounded-sm border-2 focus:border-blue-600" type="password" name="password" placeholder="Senha" required>
    </div>
